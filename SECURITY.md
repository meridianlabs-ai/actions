...
- The `analyze` and `agent` jobs hold only the read-only job token. Their
  ...
  use the `ANTHROPIC_API_KEY` the test suites run with) are read before
  the agent starts by the model broker (`.github/actions/model-broker`), a
  process under its own Unix user that forwards Messages API calls to
  `api.anthropic.com` and nothing else. The agent's `ANTHROPIC_API_KEY` is
  the broker's per-run token, which works only against the broker on the
  runner's loopback interface while the job runs, so disclosing it yields
  nothing reusable. A check that runs as the runner user after
  harden-runner fails the job before the agent starts if sudo or containers
  still work, if `ptrace_scope` would let a process read its ancestors'
  memory, or if the broker's files or `/proc` entries are readable. The
  triage agent's tools are reads plus file writes under the landing
  directory; the writes it wants are a manifest that the `land` job
  validates and performs.
...
- Both agent jobs run under harden-runner's egress allow-list (Anthropic,
  ...
  the Python package indexes) with sudo and containers disabled. The
...
- The agent can spend the capped workspace budget through the broker for as
  long as its job runs; the cap bounds it. The runner's own process holds
  the job's secrets in memory (GitHub Actions' design); the isolation check
  requires `kernel.yama.ptrace_scope` of 1 or stricter, which denies a
  process the agent starts access to its ancestors' memory.
...
2. Write credentials (app secrets, tokens, webhooks) stay out of any job
   ...
   produced. The model key goes to `.github/actions/model-broker`, started
   before harden-runner, and the agent gets the broker's token and base
   URL; no step after the agent holds a secret.
